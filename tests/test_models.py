...
class UserManagerTestCase(TransactionTestCase):
    ...
    def test_create_user(self):
        for case in (None, False, True):
            with self.subTest(case=case):
                ...
                user = User.objects.create_user(instance.name, instance.tel, "password", **extra_fields)
        ...
        for case in (None, True):
            with self.subTest(case=case):
                ...
                user = User.objects.create_superuser(
                    instance.name, instance.tel, "password", **extra_fields if case else {}
                )
    ...
    def test_user_unique_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            User.objects.create_user(user.name, user.tel, "password", **{field: True for field in self.fields})

    def test_user_tel_validator(self):
        for tel in ("asdfd", "8811109", "823892839829", "89xx8299"):
            with self.subTest(tel=tel):
                with self.assertRaises(IntegrityError):
                    User.objects.create_user("John Doe", tel, "password", **{field: True for field in self.fields})
